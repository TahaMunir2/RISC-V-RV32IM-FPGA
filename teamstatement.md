...
![Single Cycle CPU Schematic](images/Modified%2520Single%2520Cycle%2520CPU%2520diagram.jpg)
...
![Full RV32I Schematic](images/schematicfullriscv.png)
...
![Pipelined Processor Schematic](images/schematicpipelining.png)
...
![FSM Diagram](images/branchp.png)
...
![Branch Prediction Schematic](images/branchpredictio.png)
...
|------|---------|---------|
| mstatus | 0x300 | Holds global enable for all types of interrupts |
| mtvec | 0x305 | Holds address of trap handler |
| mepc | 0x341 | Holds return address from trap handler |
| mcause | 0x342 | Holds the reason for the trap |
| mip | 0x344 | Holds pending interrupts |
| mie | 0x304 | Holds interrupt specific enables |
| mscratch | 0x340* | Scratch register for general purpose use |
> *For the FPGA, we simplified our array to 7 registers and mapped all extra addresses to the scratch register

#### New Modules:
- A timer module for the timer interrupts
- A GPIO module for the DE-10 Lite LEDs
- A 7-segment module to display the bottom 3 bytes of a0
- A debouncer module to clean the button presses
- An FPGA wrapper module to connect I/O to our CPU

#### Key Design Decisions
- We used Memory Mapped Inputs and Outputs by allocating unused addresses in the memory map to the timer and LEDs.
- We made combinational logic to prevent both interrupts from conflicting with each other.
- We had to change the RAM and ROM read to synchronous to allocate BRAM on the FPGA to it.
- We had to update the Hazard and pipelining logic to deal with synchronous reading.
- We operated under the assumption that our CPU is always running in machine mode.
- We made trigger our external interrupt.
- We removed branch prediction due to the newly added complexity with synchronous read and M instructions, due to inefficiencies in trying to implement them as single-cycle instructions.

Our FPGA was able to run at the full 50 MHz and was composed of 4249 logic elements and 1955 registers, which shows the sophistication of our design.

### Schematic:

![diagram](images/FPGA_schematic.png)
...
![OOO Full Schematic](images/ooofull_2.png)
