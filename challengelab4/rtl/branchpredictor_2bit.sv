module branchpredictor_2bit #(
    ...
    input  logic enable,//enable signal for the FSM
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            for (int i = 0; i <TARGET_BUFFER_SIZE; i++)
                array[i] <= next_array[i]; //we update the target buffer
        end
    end

    always_comb begin
        // default: we keep all current states
        for (int i = 0; i <TARGET_BUFFER_SIZE;i++)
        ...
        if (enable) begin
            case (array[update_index])
                ...
                STRONGLY_NOT_TAKEN: begin
                    next_array[update_index] =actual_taken ? WEAKLY_NOT_TAKEN : STRONGLY_NOT_TAKEN;
                end

                WEAKLY_NOT_TAKEN: begin
                    next_array[update_index] = actual_taken ? WEAKLY_TAKEN : STRONGLY_NOT_TAKEN;
                end

                WEAKLY_TAKEN: begin
                    next_array[update_index] = actual_taken ? STRONGLY_TAKEN : WEAKLY_NOT_TAKEN;
                end

                STRONGLY_TAKEN: begin
                    next_array[update_index] =actual_taken ? STRONGLY_TAKEN : WEAKLY_TAKEN;
                end

                default: begin
                    next_array[update_index] = STRONGLY_NOT_TAKEN;
                end
