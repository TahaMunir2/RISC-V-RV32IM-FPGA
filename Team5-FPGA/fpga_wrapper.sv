module fpga_wrapper(
    ...
    logic rst_n_meta;  // First stage flip-flop
    logic cpu_rst;     // Second stage(clean synchronous reset

    always_ff @(posedge cpu_clk) begin
        rst_n_meta <= ~KEY[0]; // Sample the physical button 
        cpu_rst    <= rst_n_meta; // Sample the sampled value
    end
  

    assign trigger_raw = ~KEY[1];
