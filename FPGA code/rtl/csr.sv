module csr #( 
    ...
    logic [2:0] mapped_address; // to translate the assembly address to our simplified array
    ...
    logic PCE_Save; // save it from flush
...
// Add && (PCE != 0)
	assign trap_en = (global_en && ((int_ext_en && int_ext_p) || (int_tim_en && int_tim_p))) && (PCE != 32'b0);    // if interrupts are enabled and there is a certain type of interrupt pending and that type is enabled then enter the trap handler
